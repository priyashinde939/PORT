...
const Card1: React.FC<CardProps> = ({ scrollRef }) => {
    ...
    return (
        <motion.div
        transition={{ type: "linear", stiffness: 50, damping: 10, ease:[0,0,0,1], delay:5, duration:10}}
            style={{ scale, opacity }}
...
const Card3: React.FC<CardProps> = ({ scrollRef }) => {
    ...
    return (
        <motion.div
            transition={{ type: "linear", stiffness: 50, damping: 10, ease:[0,0,0,1], delay:2, duration:10}}
            style={{ scale, opacity }}
            className="sticky top-0 flex items-center justify-center"
        >
