...
import s from './animatedSvg.module.scss';
...
const AnimatedSVG: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      if (!pathRef.current || !svg) return;

      const rect = svg.getBoundingClientRect();
      const viewportHeight = window.innerHeight;

      // Calculate how much of the SVG is visible in the viewport
      const visibleRatio = Math.max(
        0,
        Math.min(1, (viewportHeight - rect.top) / (viewportHeight + rect.height))
      );

      // Adjust strokeDashoffset based on the visible ratio
      const newDashOffset = pathLength * (1 - visibleRatio);
      pathRef.current.style.strokeDashoffset = `${newDashOffset}`;
    };
